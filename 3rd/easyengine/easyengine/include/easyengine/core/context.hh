...
private:
    ...
    // push_device和pop_device只能在同一个线程中调用，且与ScopeDevice一起使用
    void push_device(int idx);
    void pop_device();

public:
    static const std::string EMPTY_STR;
    // 显式构造函数，接受一个ContextImpl的唯一指针
    ...
    // 如果idx与active_device()不同，则释放当前设备并分配设备idx
    ...
    int rank() const; // 编号
    int world_size() const; // 总进程数
    int get_compute_capability() const;
    int get_mp_count() const; // 获取多处理器数量
    ...
    // 如果active_device()与张量不同，则将张量缓存到active_device()
...
// 如果启用了调试，则创建并记录cudaEvent_t以在当前作用域内测量时间
struct EventScope {
    const Context& ctx;
    int debug_level;
    std::string end_name;
